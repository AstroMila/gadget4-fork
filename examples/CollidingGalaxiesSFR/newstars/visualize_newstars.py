#!/usr/bin/env python3
"""
Visualize only newly formed stars during galaxy collision
Stars appear from nothing as they form from gas
"""
...
def load_snapshot(filename):
    """Load only newly formed star data from HDF5 snapshot"""
    with h5py.File(filename, 'r') as f:
        ...
        # Load ONLY newly formed stars (PartType4)
        if 'PartType4' in f:
...
def create_animation(output_dir='../output', save_file='newstars_only.mp4', fps=2):
    ...
    # Determine coordinate limits from snapshots with new stars
    all_x = []
    all_y = []
    ...
    if len(all_x) == 0:
        print("No new stars found in any snapshot!")
        return
    ...
    # Initialize empty scatter plot - using bright colors for new stars
    new_stars = ax.scatter([], [], c='cyan', s=2.0, alpha=0.9, edgecolors='white', linewidths=0.1)
    
    # Time text
    time_text = ax.text(0.02, 0.98, '', transform=ax.transAxes, 
                       color='white', fontsize=16, verticalalignment='top', weight='bold',
                       bbox=dict(boxstyle='round', facecolor='black', alpha=0.7, edgecolor='cyan'))
    
    # Star count text
    count_text = ax.text(0.02, 0.91, '', transform=ax.transAxes,
                        color='cyan', fontsize=14, verticalalignment='top', weight='bold',
                        bbox=dict(boxstyle='round', facecolor='black', alpha=0.7, edgecolor='cyan'))
    
    # Title
    title_text = ax.text(0.5, 0.98, 'Newly Formed Stars Only', transform=ax.transAxes,
    ...
    def init():
        ...
        time_text.set_text('')
        count_text.set_text('')
        return new_stars, time_text, count_text, title_text
    
    def animate(frame):
        snap = snapshots[frame]
        
        # Plot ONLY newly formed stars
        if len(snap['newstars_pos']) > 0:
        ...
        count_text.set_text(f"Stars formed: {len(snap['newstars_pos']):,}")
...
def create_static_frames(output_dir='../output', frames_dir='frames'):
    ...
    print(f"Creating {len(snapshot_files)} static frames (new stars only)...")
    ...
    for i, (sfile, snap) in enumerate(zip(snapshot_files, snapshots)):
        ...
        if len(snap['newstars_pos']) > 0:
            ax.scatter(snap['newstars_pos'][:, 0], snap['newstars_pos'][:, 1],
                      c='cyan', s=2.0, alpha=0.9, edgecolors='white', linewidths=0.1)
        
        # Add title
        ax.text(0.5, 0.98, 'Newly Formed Stars Only', transform=ax.transAxes,
               color='white', fontsize=18, verticalalignment='top',
               horizontalalignment='center', weight='bold',
               bbox=dict(boxstyle='round', facecolor='black', alpha=0.7, edgecolor='white'))
        ...
        # Add count text
        ax.text(0.02, 0.91, f"Stars formed: {len(snap['newstars_pos']):,}",
    ...
    print(f"You can create a video with: ffmpeg -framerate 2 -i {frames_dir}/frame_%03d.png -c:v libx264 -pix_fmt yuv420p newstars_only.mp4")
...
if __name__ == '__main__':
    ...
    print("=" * 60)
    print("New Stars Only - Galaxy Collision Visualization")
    print("=" * 60)
